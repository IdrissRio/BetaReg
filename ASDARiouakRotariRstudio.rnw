...
\end{itemize}
L'opzione \texttt{control} prende come parametro un oggetto di tipo \texttt{betareg.control}. Tali oggetti servono per controlare la modalità con la quale veogno stimati i coefficienti del modello.
\begin{lstlisting}
...
\caption{A sinistra la fitted curves del modello di regressione lineare. A destra il plot diagnostisco rappresentante  i fitted values rispetto la radice quadrata del valore assoluto dei residui standardizzati.}
\end{figure}
