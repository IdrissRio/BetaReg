...
\hspace{1cm}\includegraphics[scale=0.4]{Spese}

Consideriamo il Modello di regressione lineare per il nostro dataset:

<<eval=TRUE>>=
data("FoodExpenditure",package = "betareg")
mod.regressione<-lm(I(food/income) ~ income, data = FoodExpenditure)
@
\hspace{1cm}\includegraphics[scale=0.48]{lm_es2}\\
...
Possiamo vedere dal p-value che l'ipotesi nulla viene rifiuta, concludiamo che il nostro modello è eteroschedastico, quindi il modello di beta regressione è più appropriato. Usiamo come funzione di collegamento quella di default
<<eval=TRUE>>=
library("betareg")
data("FoodExpenditure",package = "betareg")
beta_regressione<-betareg(I(food/income) ~ income, data = FoodExpenditure)
summary(beta_regressione)
@
\hspace{1cm}\includegraphics[scale=0.55]{lm_beta}\\

Mettendo a confronto entrambi i modelli possiamo vedere che in realtà non differiscono molto. Le fitted curves differiscono di poco una dall'altra. 
Da entrambi i modelli possiamo concludere che la percentuale



la percentuale del reddito delle famiglie speso in cibo diminuisce con il livello di reddito complessivo ma aumenta nel numero di persone nella famiglia
